...
# def PlotOfertaDemanda_categoria_medsum(tipo_categoria,eixoy,serie_datas,matriz_demanda,matriz_carac,titulo_grafico):
#     matriz_demanda = matriz_demanda[372:,1:]
#     for i in range(0, len(matriz_demanda)):
#         matriz_demanda = [np.nan if i > 2.5e5 else float(i) for i in
#                           matriz_demanda]  # filtrando dados de volume muito altos: outliers
#         # if eixoy2 == 'Volume consumido (m³)':
#         #     matriz_demanda = [1 if i == 0 else float(i) for i in
#         #                      matriz_demanda]
...
#     #     if categoria == 'CP' or categoria == 'D3':
#     #         c = minha_cor(categoria)[0]
#     #         ms = minha_cor(categoria)[1]
#     #         plt.plot(serie_datas, list(matriz_demanda[:, j]), 'o', linestyle='None',label=str(categoria),markerfacecolor="None",
#     #          markeredgecolor=c, markeredgewidth=0.25,markersize=ms)
#     #     if categoria == 'CI' or categoria == 'D2':
#     #         c = minha_cor(categoria)[0]
#     #         ms = minha_cor(categoria)[1]
#     #         plt.plot(serie_datas, list(matriz_demanda[:, j]), 'o', linestyle='None',label=str(categoria),markerfacecolor="None",
#     #          markeredgecolor=c, markeredgewidth=0.25,markersize=ms)
...
# def PlotOfertaDemanda_categoria_medsum_duplo(tipo_categoria,tipo_dado,eixoy1,eixoy2,serie_datas,serie_oferta,matriz_demanda,matriz_carac,titulo_grafico):
...
def PlotOfertaDemanda_medsum(tipo_dado,eixoy1,eixoy2,serie_datas,serie_oferta,matriz_demanda,titulo_grafico):
    ...
    serie_oferta = [np.nan if i == "None" else float(i) for i in serie_oferta]
    matriz_demanda = matriz_demanda[372:,1:]
    matriz_demanda = matriz_demanda.astype(np.object)
    lim_outliers = 2.5e5
    for i in range(0, len(matriz_demanda)):
        for j in range(0, 37): #37 columns, from 0 to 36
            if matriz_demanda[i, j] == 'None':
                ...
                matriz_demanda[i, j] = float(matriz_demanda[i,j])

    for i in range(0, len(matriz_demanda)):
        for j in range(0, 37): #37 columns, from 0 to 36
            if matriz_demanda[i, j] != np.nan:
                if matriz_demanda[i, j] > lim_outliers:# filtrando dados de volume muito altos: outliers
                    matriz_demanda[i, j] = np.nan
    fig = plt.figure()
    gs = gridspec.GridSpec(2, 1, height_ratios=[1, 1])  # plota vários gráficos numa mesma figura
    # Oferta
    ...
    for i in range(0,len(matriz_demanda)):
        vetor_soma_d.append(np.nansum(matriz_demanda[i,0:36])) #considerando nan como zero
        vetor_media_d.append(np.nanmean(matriz_demanda[i,0:36],dtype=float)) #considerando nan como zero
    ax.plot(serie_datas, vetor_media_d, 'o', linestyle='None',label='Média diária',markerfacecolor="None",
         markeredgecolor='darkgray', markeredgewidth=1.0,markersize=5)
    ax.plot(serie_datas, vetor_soma_d, 'o', linestyle='None',label='Soma diária',markerfacecolor="black",
         markeredgecolor='white', markeredgewidth=0.5,markersize=3)
    ax.set_ylabel(eixoy2, color='black')
    ax.tick_params(axis='y', colors='black')
    ax.xaxis.grid(b=True, which='major', color='.9', linestyle='--') # linhas de grade do eixo x
    ax.yaxis.grid(b=True, which='major', color='.9', linestyle='--') # linhas de grade do eixo y
...
###Volume consumido
####Chuva 26798500
serie_oferta = serie_chuva2
matriz_demanda = matrix_volume_dem
titulo_grafico = 'Chuva 26798500 x Volume consumido'
tipo_dado = 'barra'
eixoy1 = 'Chuva (mm)'
...
titulo_grafico = 'Chuva 26798500 x Vazão consumida'
tipo_dado = 'barra'
eixoy1 = 'Chuva (mm)'
